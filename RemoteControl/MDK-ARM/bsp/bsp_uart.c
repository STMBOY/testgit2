...
/* for embedded test */
uint8_t   test_buf[TEST_BUFLEN];
 char   test_tx_buf[200];
...
{
	/* open uart idle it */
	__HAL_UART_CLEAR_IDLEFLAG(&TEST_HUART);
...
/**
  ...
  * @retval  
  */
static void test_uart_rx_idle_callback(UART_HandleTypeDef* huart)
{
	/* clear idle it flag avoid idle interrupt all the time */
	__HAL_UART_CLEAR_IDLEFLAG(huart);
...
//		/* handle dbus data dbus_buf from DMA */
//		if ((TEST_MAX_LEN - dma_current_data_counter(huart->hdmarx->Instance)) == TEST_BUFLEN)
//		{
			test_callback_handler(test_buf);	
//		}
...
void test_uart_receive_handler(UART_HandleTypeDef *huart)
{
	if (__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) && 
			__HAL_UART_GET_IT_SOURCE(huart, UART_IT_IDLE))
	{
		test_uart_rx_idle_callback(huart);
	}
}

void test_callback_handler(uint8_t *buff)
{
	if (buff[0] == 0xAA)  //命令码是测试
	{
		switch (buff[1])
		{
		case TEST_RC_CALLBACK_HANDLER:
		{
			memcpy(dbus_buf, (buff+2), 18);
			rc_callback_handler(&rc, dbus_buf);	
			sprintf(test_tx_buf, "CH1: %4d  CH2: %4d  CH3: %4d  CH4: %4d  SW1: %1d  SW2: %1d ", rc.ch1, rc.ch2, rc.ch3, rc.ch4, rc.sw1, rc.sw2);
			HAL_UART_Transmit(&huart6, (uint8_t *)test_tx_buf, (COUNTOF(test_tx_buf) - 1), 55);
		} break;
